...
//在特定目录下，创建文件
export const addFileInDir = (fileName: string, dirPath: string, data: any): boolean => {
...
export const copyDir = (srcPath: string, tarPath: string, filter: string[] = []): void => {
    const tarDirpath = path.join(tarPath)
    if (!fs.existsSync(tarDirpath)) fs.mkdir(path.join(tarDirpath), (err: any) => { });
    fs.readdir(srcPath, (err: NodeJS.ErrnoException | null, files: string[]) => {
        ...
        files.forEach(function (filename: string) {
            ...
            if (!filterFlag) {
                fs.stat(filedir, function (errs: NodeJS.ErrnoException | null, stats: { isFile: () => any; }) {
                    ...
                    if (isFile) { // 复制文件
                        ...
                        copyTemplate(filedir, tarFiledir, filter); // 递归
                    }
                });
            }
        });
